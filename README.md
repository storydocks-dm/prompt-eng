...
## Deploying the course to Schüttflix Infrastructure

The course contains Jupyter Notebooks that will run in AWS Sagemaker and connect to AWS Bedrock
To deploy the course, make sure you are logged into our AWS accounts:

```
aws sso login
```

Then you can deploy the CloudFormation stack using AWS CLI:

```
  aws cloudformation deploy \
    --template-file AmazonBedrock/cloudformation/workshop-v1-final-cfn.yml \
    --stack-name prompt-eng-tutorial-sebastian \
    --parameter-overrides NotebookName=PromptEngNotebook-sebastian \
